  ...
  for (const u of users) {
    ...
    try {
      ...
      if (e.code === 'auth/email-already-in-use') {
        try {
          cred = await signInWithEmailAndPassword(secondaryAuth, u.login, 'N3xus2026!');
          console.log(`Signed In: ${u.name}`);
        } catch (e2) {
          try {
            cred = await signInWithEmailAndPassword(secondaryAuth, u.login, 'spiral123!');
            console.log(`Signed In: ${u.name}`);
          } catch(e3) {
            console.error(`Sign in failed for ${u.name}`);
          }
        }
      } else {
