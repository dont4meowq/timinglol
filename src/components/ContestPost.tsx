...
import { Edit2, Trash2, Heart, MessageSquare, Send, X } from "lucide-react";
import { useStore } from "../store";
import { db } from "../firebase";
import { collection, query, where, onSnapshot, addDoc, deleteDoc, doc, updateDoc } from "firebase/firestore";

export 
function ContestPost({ contest, isAdmin, onEdit }: { key?: string, contest: any, isAdmin: boolean, onEdit: () => void }) {
  const { currentUser, toggleContestLike, deleteContest, allUsers } = useStore();
  const [showComments, setShowComments] = useState(false);
  const [zoomedImage, setZoomedImage] = useState<string | null>(null);

  const isLiked = currentUser && contest.likes.includes(currentUser.id);
  const likedByNames = contest.likes.map((id: string) => allUsers.find(u => u.id === id)?.name || 'Неизвестный');
  ...
  return (
    <article className="bg-[#252526] border border-neutral-800 rounded-xl overflow-hidden shadow-sm">
        ...
        <div className="flex items-start justify-between mb-4">
          <div>
            ...
            <div className="flex items-center gap-2 text-sm text-neutral-400">
              <span className="text-blue-400 font-medium">{contest.authorName}</span>
              <span>•</span>
              <span>{new Date(contest.createdAt || Date.now()).toLocaleDateString('ru-RU', { day: 'numeric', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</span>
            </div>
          </div>
          {isAdmin && (
      ...
      {showComments && (
        <CommentsSection contestId={contest.id} />
      )}
...
function CommentsSection({ contestId }: { contestId: string }) {
  ...
  useEffect(() => {
    const q = query(
      collection(db, 'contest_comments'), 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await addDoc(collection(db, 'contest_comments'), {
        contestId,
  ...
  const handleDelete = async (id: string) => {
    await deleteDoc(doc(db, 'contest_comments', id));
  };
  ...
  const handleLikeComment = async (commentId: string, currentLikes: string[] = []) => {
    ...
    await updateDoc(doc(db, 'contest_comments', commentId), { likes: newLikes });
  };
  ...
  const handleSaveEditComment = async (commentId: string) => {
    ...
    await updateDoc(doc(db, 'contest_comments', commentId), {
